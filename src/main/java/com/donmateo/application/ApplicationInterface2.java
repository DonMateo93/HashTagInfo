 ...
 */
public class ApplicationInterface2 {
    ...
    public ApplicationInterface2(SessionFactory factory) {
    ...
    public int begin() throws IOException {
        ...
        while (!back) {
            ...
            printWhereAreYou();
            CommandType command = userPolling();
            switch (command) {
                case NONE:
    ...
    public CommandType userPolling() throws IOException {
    ...
    public CommandType parseCommand(String command) {
        ...
        if (command.length() > 0) {
            command_type = Command.parseString(Arrays.asList(command.split("\\s*,\\s*")).get(0));
            if (Command.isCommandAvailibleInState(command_type, app_state)) {
                updateCommandStr(command);
            } else {
